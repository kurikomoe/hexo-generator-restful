...
// small container
md.use(require('markdown-it-container'), 'ref', {});

// containers
md.use(require('markdown-it-container'), 'note', {});
md.use(require('markdown-it-container'), 'info', {});
md.use(require('markdown-it-container'), 'extra', {});
md.use(require('markdown-it-container'), 'warning', {});
md.use(require('markdown-it-container'), 'alert', {});

md.use(require('markdown-it-container'), 'example', {}); // should be same as extra

// extra
md.use(require('markdown-it-container'), 'question', {});
md.use(require('markdown-it-container'), 'define', {});
md.use(require('markdown-it-container'), 'explain', {});
md.use(require('markdown-it-container'), 'theorem', {});

// special
md.use(require('markdown-it-container'), 'quote', {});
...
module.exports = function (cfg, site) {
  function cmpfunc(x, y) {
    return new Date(y.updated) - new Date(x.updated)
      ...
      || (y.title && x.title) && pinyin(y.title.toString()).join(' ').localeCompare(pinyin(x.title.toString()).join(' '))
      || (y.subtitle && x.subtitle) && pinyin(y.subtitle.toString()).join(' ').localeCompare(pinyin(x.subtitle.toString()).join(' '));
  }
  ...
  var restful = cfg.hasOwnProperty('restful') ? cfg.restful :
    ...
    posts = site.posts.toArray().sort(cmpfunc).filter(function (post) {
      if (process.env.HEXO_IS_SHOW_DRAFT === '1') {
        return true;
      } else {
